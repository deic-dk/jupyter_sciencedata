...
def _get_directory(context, path, content):
    files = webdav_client.list("files/" + path, get_info=True) if content else []
    return {
        'name': _final_path_component(path),
        'path': path,
        'type': 'directory',
...
def _list_checkpoints(context, path):
    files = webdav_client.list("files/" + path, get_info=True)
    return [
        {
...
def _make_sciencedata_http_request(context, method, path, query, payload, headers):
    ...
    encoded_path = urllib.parse.quote("files/" + path, safe='/~')
